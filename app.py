...
DOWNLOAD_DIRS = {
    ...
    "Séries": "series_baixadas"  # <-- adicionado diretório para séries
}
...
def set_background(png_file, config):
    pass  # Função desativada para remover wallpapers
...
def pagina_series():
    st.title("📺 Séries — Buscar, Assistir ou Baixar")

    with st.form(key="series_search_form"):
        query = st.text_input("Pesquisar série/episódio (ex: 'Nome S01E01')", placeholder="Ex: Stranger Things S01E01")
        submitted = st.form_submit_button("Pesquisar")
    if submitted:
        if not query:
            st.warning("Digite o nome/episódio da série.")
            return
        ...
        if not entries:
            st.info("Nenhum episódio/encontro encontrado.")
            return
        ...
        for i, entry in enumerate(entries):
            ...
            with col2:
                ...
                st.caption(f"Duração: {duration}")
            with col3:
                if st.button("Assistir Online", key=f"watch_series_{i}"):
                    # Tenta obter uma URL de streaming direta e reproduzir com st.video
                    try:
                        with st.spinner("Carregando stream..."):
                            with yt_dlp.YoutubeDL({'http_headers': {'User-Agent': 'Mozilla/5.0'}}) as ydl:
                                full = ydl.extract_info(entry.get('url') or entry.get('webpage_url') or entry.get('id'), download=False)
                                # escolhe o melhor formato progressivo/HTTP disponível
                                formats = full.get('formats') or []
                                stream_url = None
                                if formats:
                                    # ordena por resolução e busca protocolo http(s)
                                    ...
                                    for f in formats_sorted:
                                        ...
                                        if proto.startswith('http'):
                                            ...
                                            break
                                # fallback para webpage_url
                                if not stream_url:
                                    stream_url = full.get('url') or full.get('webpage_url')
                                if stream_url:
                                    st.video(stream_url)
                                else:
                                    st.error("Não foi possível obter URL de streaming.")
                    except Exception as e:
                        st.error(f"Erro ao obter stream: {e}")

                if st.button("Baixar Episódio", key=f"download_series_{i}"):
                    # adiciona à fila usando gerenciador_de_download
                    episode_title = entry.get('title') or f"Série_{i}"
                    ydl_opts = {
                        'outtmpl': f'{DOWNLOAD_DIRS["Séries"]}/%(title)s.%(ext)s',
                        'ffmpeg_location': FFMPEG_LOCATION,
                        'http_headers': {'User-Agent': 'Mozilla/5.0'},
                    }
                    # Garante diretório
                    os.makedirs(DOWNLOAD_DIRS["Séries"], exist_ok=True)
                    gerenciador_de_download(ydl_opts, entry.get('url') or entry.get('webpage_url') or entry.get('id'), "Séries", DOWNLOAD_DIRS["Séries"], display_mode='full')
...
elif pagina == "Séries":  # <-- novo roteamento
    pagina_series()
